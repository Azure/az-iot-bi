...
function getSettingFilepath(fileFolder) {
  if (fileFolder) {
    biSettingDirPath = fileFolder;    
  } else {
    biSettingDirPath = path.join(userHome, '.iot-hub-getting-started');
  }
  biSettingFilePath = path.join(biSettingDirPath, 'biSettings.json');

  return { "biSettingDirPath" : biSettingDirPath, "biSettingFilePath" : biSettingFilePath };
}
...
biSetting.readSetting = function (userBiSettingDir) {
  biSettingFilePath = getSettingFilepath(userBiSettingDir).biSettingFilePath;
  return readSetting(biSettingFilePath);
};

biSetting.writeSetting = function (settings, userBiSettingDir) {
  try {
    var res = getSettingFilepath(userBiSettingDir);
    biSettingDirPath = res.biSettingDirPath;
    biSettingFilePath = res.biSettingFilePath;
...
biSetting.isBIEnabled = function (skipCheck, userBiSettingDir) {
  if (skipCheck === true) {
    return true;
  }
